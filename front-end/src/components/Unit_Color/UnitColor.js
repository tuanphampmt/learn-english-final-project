      ...
      if (e) {
        ...
        if (e.style.cursor === "pointer") {
          if (this.state.color === color) {
            ...
            var url = this.state.urls.find((item) => item.code === color);
            if (url) {
              this.setState({ url: url.name });
            }
            var colors = this.state.colors.filter((i) => i !== color);
            this.setState({
              colors,
            });
            ...
            Promise.resolve(3000)
              ...
              .then(() => {
                if (this.state.colors.length !== 0) {
                  var color = this.random(this.state.colors, 1)[0];
                  this.setState({
                    color,
                    url: "/Images/Unit Color/Game_Card/Pig/NoColor_Pig.png",
                  });
  ...
  render() {
    return (
      <div className="container unit-aphabet">
        ...
        <span id="Point">Điểm: {this.state.score}</span>
              </div>
